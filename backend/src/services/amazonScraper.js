...
export const fetchDealsByCategory = async (
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      logger.info(
        `Fetching deals for category: ${category} (attempt ${attempt}) - PA-API call`
      );
      ...
      logger.error(
        `PA-API Error for ${category} (attempt ${attempt}): ${error.message}`
      );
  ...
  logger.error(
    `Failed to fetch deals for ${category} after ${retries} attempts.`
  );
  return [];
};
