...
const CSV_HEADERS = [
    ...
    { id: "RatingsCount", title: "Ratings Count" },
...
export const exportDealsToCsv = async (records) => {
    ...
    try {
        ...
        logger.debug(`Ensured directory exists: ${outputDir}`);
    } catch (e) {
        logger.error(`❌ CRITICAL: Failed to create export directory: ${e.message}`);
        return null;
    }
    ...
    try {
        ...
        logger.info(
            `✅ Successfully exported ${records.length} deals to: ${filePath}`
        );
        return filePath;
    } catch (error) {
