...
const extractCurrency = (str) =>
  str.match(/[₹$€£]/)?.[0] || config.AMAZON.CURRENCY || "INR";
...
const cleanAndValidateProduct = (rawProduct, category) => {
  try {
    ...
    if (
      !listing ||
      !listing.Price ||
      !rawProduct.ItemInfo?.Title?.DisplayValue
    ) {
      logger.debug(
        `Skipping ASIN ${asin}: Missing essential data (price or title).`
      );
      return null;
    }
    ...
    if (currentPrice === null) {
      logger.debug(
        `Skipping ASIN ${asin}: Could not parse valid current price from ${priceStr}.`
      );
      return null;
    }
    ...
    if (discountPercentage < config.MIN_DISCOUNT_PERCENT) {
      logger.debug(
        `Skipping ASIN ${asin}: Discount (${discountPercentage}%) below threshold.`
      );
      return null;
    }

    const brand = rawProduct.ItemInfo?.ByLineInfo?.Brand?.DisplayValue || null;
    const isPrimeEligible = listing?.DeliveryInfo?.IsPrimeEligible || false;
